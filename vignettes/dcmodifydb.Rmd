...
A better approach is to store the rules separately and apply them to a datatable.
This makes it easier to maintain a set of rules. 
...
Note that the rules are executed in sequentially, in the order that they are given.
...
Rule M5 shows an example of deriving a new variable (`ageclass`) from existing 
variables (`age`).

Rule M6 shows an example of recoding, changing labels of categories to the allowed
set of labels.
