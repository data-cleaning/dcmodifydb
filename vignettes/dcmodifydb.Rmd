...
```{r}
library(dcmodify) # needed for specifying correction rules
library(dcmodifydb) # needed to translate the rules into SQL
modify( person_tbl,
...
accidents with the data and that we need to specify with `key` the column(s) that identify 
a row in the database table.

A better approach than directly applying corrections is to store the rules 
in a `modifier` object first and apply them in a separate step to a data base table.
...
modify(person_tbl, m, key="id", copy = TRUE)
```
...
```{r dump, eval=FALSE}
dump_sql(m, person_tbl, key = "id", file = "corrections.sql")
```

_corrections.sql_:
```SQL
```{r, eval=TRUE, echo=FALSE, results='asis'}
dump_sql(m, person_tbl, key = "id")
```
